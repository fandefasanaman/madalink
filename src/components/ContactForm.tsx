...
const ContactForm: React.FC<ContactFormProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    const contact: Omit<Contact, 'id' | 'notes'> = {
      ...
      createdAt: new Date(),
      // Champs CRM requis avec valeurs par défaut
      totalSpent: 0,
      averageOrderValue: 0,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/80 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">Ajouter un nouveau client</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl transition-all touch-manipulation text-sm sm:text-base ${
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              ...
              <button
                onClick={addTag}
                className="px-4 py-2 bg-sage-600 hover:bg-sage-700 text-white rounded-lg transition-colors touch-manipulation text-sm sm:text-base"
              >
                Ajouter
              </button>
            </div>
          ...
          <div className="flex items-center space-x-3">
            <input
              ...
              className="w-4 h-4 text-sage-600 bg-gray-100 border-gray-300 rounded focus:ring-sage-500 dark:focus:ring-sage-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 touch-manipulation"
            />
            <label htmlFor="starred" className="text-sm font-medium text-gray-700 dark:text-gray-300 touch-manipulation">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end space-y-2 sm:space-y-0 sm:space-x-3 p-4 sm:p-6 border-t border-gray-200 dark:border-gray-800">
          ...
          <button
            onClick={handleSave}
            disabled={!formData.name.trim() || isSaving || !currentUser}
            className={`flex items-center justify-center space-x-2 px-6 py-3 sm:py-2 rounded-lg font-medium transition-all touch-manipulation ${
              !formData.name.trim() || isSaving || !currentUser
          ...
          >
            ...
            <span>{isSaving ? 'Enregistrement...' : 'Ajouter le client'}</span>
          </button>
        </div>
      </div>
    </div>
